...
import io.skjaere.debridav.usenet.UsenetRepository
...
class SabnzbdApiController(
    ...
    private suspend fun addNzbFile(request: SabnzbdApiRequest): String {
        ...
        if (debridFiles.isNotEmpty()) {
            ...
            val savedUsenetDownload = createCachedUsenetDownload(
                releaseName,
                request,
                createdFiles.map { fileService.getDebridFileContents(it) }
            )
            return Json.encodeToString(
